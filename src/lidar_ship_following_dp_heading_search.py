...
class Control_Param():
    ...
    def sub_local_xy(self,msg):
        if len(msg.data) > 3:
            ...
            for i in range(N):
                if self.ID == int(msg.data[1+6*i]):
                    self.real_rel_x = msg.data[1+6*i+1]
                    self.real_rel_y = msg.data[1+6*i+2]
                    global_lidar_heading = -msg.data[1+6*i+5]
    ...
    # def sub_global_vel(self,msg):
    #     if len(msg.data) > 3:
    #         vx = msg.data[6]
    #         vy = msg.data[7]
    #         self.global_vel_x = vx
    #         self.global_vel_y = vy
    #         self.Target_vessel_pose_yaw = 0.0

    #         # if math.sqrt(vx*vx + vy*vy) > 1.0:
    #         #     self.Target_vessel_pose_yaw = math.atan2(vy,vx)

            
    #         # self.Target_vessel_pose_yaw = math.atan2(vy,vx)

    #             # print("rel yaw : " ,self.rel_yaw)
    #             # print("vx : " ,vx, ", vy : ", vy)
    ...
    def control_time(self,msg):
        ...
        ######################################################################## heading search #########################################################################
                ...
                if self.found_heading == 0:
                    ...
                    #####################################################
                    if self.heading_search_time <= 8.0:
                        if self.target_dtime > 2.0:
                            # print(self.target_dtime)
                            ...
                            if self.rel_x != self.rel_x_before and self.rel_y != self.rel_y_before:
                                ...
                                print("#####################################################################################")
                                print("Relative : " , self.heading*180/math.pi, ", Lidar : ", self.lidar_heading*180/math.pi)
        ...
        #################################################################################################################################################################
                    ...
                    if self.rel_yaw < 0.0 and self.rel_yaw > -160*math.pi/180:
                        ...
                        self.wpt_x_avoidance = (self.rel_x + (self.dock_distance-1)*math.cos(self.rel_yaw - math.pi))
                        self.wpt_y_avoidance = (self.rel_y + (self.dock_distance)*math.sin(self.rel_yaw - math.pi))
        ...
        ########################################################################## wpt control ##########################################################################
                    ...
                    wpt_x = (self.rel_x + (self.avoid_distance-1)*math.cos(self.rel_yaw - math.pi*1.5))
                    wpt_y = (self.rel_y + (self.avoid_distance)*math.sin(self.rel_yaw - math.pi*1.5))
                    ...
                    print("wpt_y" , wpt_y)
                    print("angle : ", self.angle_e)
                    print("rel : ", self.rel_yaw*180/math.pi)
        ...
        ########################################################################## 2nd control ##########################################################################
                        ...
                        if self.con_time > 1:
                            ...
                            #################################################### yaw_e -> T_con #################################################################
                            if abs(self.desired_yaw) > math.pi*40/180:
                                self.T_con = 0 
                            #####################################################################################################################################
...
def main():
    ...
    # rospy.Subscriber('/global_track', Float32MultiArray, param.sub_global_vel)
