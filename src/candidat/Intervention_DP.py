...
class Control_Param():
        ...
        if len(msg.data) > 3:
            ...
            for i in range(N):
                if self.ID == int(msg.data[1+8*i]):
                    ...
                    self.target_vessel_length = msg.data[1+8*i+6]
                    ################################################################## -- heading_on
    ...
    def control_time(self,msg):
        ...
        ########################################################################## 2nd control ##########################################################################
                        ...
                        if self.con_time > 1:
                            print("Time : ",self.con_time)
                            if self.e_x > 2.5 or self.e_x < -2.5:
                                self.switch = 1
                                self.con_time = 0.0
                        
                            if self.con_time > 3 and self.converge_button == 0:
                                print("adfassssssssssssssssssssssssssssssssssssssssssssssssffff")
